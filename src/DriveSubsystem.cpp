...
DriveSubsystem::DriveSubsystem()
    : driveTurnkP("Drive Turn P Value", .05),
      ...
      m_leftDriveShifter(DRIVE_SHIFTER_PCM, DRIVE_SHIFTER_HIGH_GEAR_PORT,
                         DRIVE_SHIFTER_LOW_GEAR_PORT),
      m_highGear(true),
...
void DriveSubsystem::teleop() {
    // Code for teleop. Sets motor speed based on the values for the joystick,
    // runs compressor, toggles gears
    double mag = -driverJoystick->GetAxis(
        CORE::COREJoystick::JoystickAxis::LEFT_STICK_Y);
    double rot = driverJoystick->GetAxis(
        CORE::COREJoystick::JoystickAxis::RIGHT_STICK_X);
...
void DriveSubsystem::SetMotorSpeed(double speedInFraction,
                                   DriveSide whichSide) {
...
void DriveSubsystem::SetMotorSpeed(double leftPercent, double rightPercent) {
    ...
    SetMotorSpeed(
        leftPercent,
        DriveSide::LEFT);  // Might need to reverse this for comp robot
...
void DriveSubsystem::InitTalons() {
    ...
    // Encoder Functions
    m_leftSlave.SetStatusFramePeriod(StatusFrameEnhanced::Status_1_General, 10,
                                     0);
    m_rightMaster.SetStatusFramePeriod(StatusFrameEnhanced::Status_1_General,
                                       10, 0);

    m_leftSlave.ConfigSelectedFeedbackSensor(
